...
def main():
    ...
    new_commit=gh(token,"POST",f"/repos/{target}/git/commits",{"message":"governance: upgrade repository-local setup to v2.6.1","tree":tree["sha"],"parents":[head]})
    ...
    print(json.dumps({"status":"LOCAL_SETUP_V2_6_1_UPGRADE_APPLIED","target":target,"old_head":head,"new_head":new_commit["sha"]}))
